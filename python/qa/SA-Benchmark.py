...
def main():
	...
	# Get a solution to TSP via the simulated annealing algorithm. #40000
	solutionList = performSimulatedAnnealing(listOfNodes, 200000, 3000, .999)
	debug("best_cost", tourDistance(
		solutionList), stop=False, key="sa_")

	end = default_timer()  # end time of running 2opt
	time = str(end - start)

	debug("time2run", time, stop=False, key="sa_")
